...
Проект следует принципам **Evolutionary Design (ED)** — подходу, при котором структура строится вокруг ответственности, а не технических типов файлов.

- **`app/`** — маршруты и композиция (Next.js App Router). Не содержит бизнес-логики.
- **`features/`** — самостоятельные функциональные модули (календарь, пресеты, настройки).
- **`shared/`** — общий фундамент: UI-компоненты, утилиты, API-клиент, хуки, хранилища состояний.
- **`packages/`** — общие типы и схемы (монорепозиторий).

Подробное описание архитектурных принципов и правил организации кода — в [ED-PROJECT-ARCHITECTURE-GUIDE.md](./ED-PROJECT-ARCHITECTURE-GUIDE.md).
...
В разделе SQL Editor выполните скрипты из папки db/:
...
│   ├── features/       # Фичи по доменам (calendar, settings, presets)
